...
if uploaded_file is not None:
    ...
    if matrix_df is not None:
        ...
        # --- DASHBOARD ---
        # Displaying the project name in the title
        st.title(f"⚰️ {project_name}: Analysis Dashboard")
        ...
        with t3:
            st.subheader(f"Raw Data Table: {project_name}")
            st.dataframe(plot_df, use_container_width=True)
            
            # Sanitizing the project name for a safe filename
            safe_filename = "".join([c for c in project_name if c.isalnum() or c in (' ', '_')]).rstrip().replace(' ', '_')
            st.download_button(
                label=f"Export {project_name} as CSV", 
                data=plot_df.to_csv(index=False), 
                file_name=f"{safe_filename}_survey_data.csv"
            )

else:
    st.info("👋 Please upload your .json file in the sidebar to begin.")
