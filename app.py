...
def load_and_process_data(uploaded_file):
    ...
    # Unique ID for tracking exclusions
...
if uploaded_file is not None:
    ...
    if matrix_df is not None:
        ...
        with t1:
            st.info("💡 Use Box/Lasso select on the map to exclude points. They will turn grey/fade.")
            
            # Map showing active and excluded points
            ...
            # This captures the selection event
            selected_points = plotly_events(fig_map, select_event=True, key="map_selection")
            
            if selected_points:
                # Identify which IDs were in the selection
        ...
        with t3:
            st.subheader("Raw Data (Excluded in Red)")
